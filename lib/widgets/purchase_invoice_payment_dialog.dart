...
class PurchaseInvoicePaymentDialog extends StatefulWidget {
  ...
  const PurchaseInvoicePaymentDialog({Key? key, required this.purchaseInvoice}) : super(key: key);

  @override
  State<PurchaseInvoicePaymentDialog> createState() => _PurchaseInvoicePaymentDialogState();
}

class _PurchaseInvoicePaymentDialogState extends State<PurchaseInvoicePaymentDialog>
  ...
  void _save() async {
    ...
    final treasuryTx = TreasuryTransaction(
      ...
      description: 'Paiement de la facture ${widget.purchaseInvoice.number}',
    ...
    double totalDue = widget.purchaseInvoice.totalTTC + widget.purchaseInvoice.timbreFiscal;
    ...
    final updatedInvoice = widget.purchaseInvoice.copyWith(
    ...
    return AppBar(
      ...
      title: Column(
        ...
        children: [
          ...
          Text('Facture d\'achat', style: TextStyle(color: Colors.white70, fontSize: 12)),
        ],
      ),
      actions: [
        Container(
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        widget.purchaseInvoice.number,
                        style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: AppColors.textPrimary),
                      ),
                      SizedBox(height: 2),
                      Text(
                        formatDateShort(widget.purchaseInvoice.date),
                        style: TextStyle(fontSize: 12, color: AppColors.textSecondary),
                      ),
                    ],
                  ),
                ),
                _buildStatusChip(widget.purchaseInvoice.status),
              ],
            ),
          ),
          Padding(
            ...
            child: Column(
              children: [
                _buildInfoRow(
                  ...
                  'Fournisseur',
                  widget.purchaseInvoice.supplierName ?? '—',
                ),
                Padding(
                  ...
                  children: [
                    Expanded(
                      child: _buildAmountBlock(
                        'Montant total',
                        formatCurrencyDT(widget.purchaseInvoice.totalTTC + widget.purchaseInvoice.timbreFiscal),
  ...
  Widget _buildStatusChip(InvoiceStatus status) {
    Color chipColor;
    String label = translateStatus(status.name);
    switch (status) {
  ...
  Widget _buildTreasuryAccountDropdown() {
    ...
    String? displayName;
    if (_selectedAccountId != null) {
      final acc = _treasuryAccounts.cast<TreasuryAccount?>().firstWhere((a) => a?.id == _selectedAccountId, orElse: () => null);
    ...
    return SearchableSelectorField(
      ...
      onTap: () async {
        final res = await showTreasuryAccountSelectDialog(context, _treasuryAccounts, selectedAccountId: _selectedAccountId);
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          Row(
                            children: [
                              Expanded(child: Text('Réf: ${widget.purchaseInvoice.number}', style: TextStyle(fontWeight: FontWeight.w500))),
                              Expanded(child: Text('Date: ${formatDateTime(widget.purchaseInvoice.date)}')),
                            ],
                          ),
                          SizedBox(height: 12),
                          Row(
                            children: [
                              Expanded(child: Text('Contact: ${widget.purchaseInvoice.supplierName ?? '—'}')),
                              Expanded(child: Text('Statut: ${widget.purchaseInvoice.status.label}')),
                            ],
                          ),
                          SizedBox(height: 12),
                          Row(
                            children: [
                              Expanded(child: Text('Montant total: ${formatCurrencyDT(widget.purchaseInvoice.totalTTC + widget.purchaseInvoice.timbreFiscal)}', style: TextStyle(fontWeight: FontWeight.w600))),
                              Expanded(
                                child: Row(
                                  children: [
                                    Text('Montant restant: ', style: TextStyle(fontWeight: FontWeight.w500)),
                                    Text(formatCurrencyDT(remainingAmount), style: TextStyle(fontWeight: FontWeight.bold, color: Colors.deepOrange)),
                                  ],
                                ),
                              ),
                            ],
                                        ...
                                        child: Row(
                                          children: [
                                            Container(
                                              ...
                                              child: Text('${t['rate']}%', style: TextStyle(color: Colors.green, fontSize: 12, fontWeight: FontWeight.bold)),
                                            ),
                                            SizedBox(width: 8),
                                            Expanded(child: Text(t['label'], overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 13))),
                                          ],
                                        ),
                                      );
                                    }).toList(),
                              ...
                              child: Column(
                                children: [
                                    ...
                                    children: [
                                      ...
                                      Expanded(
                                        child: _buildFormField('Montant', TextField(
                                          controller: _amountCtrl,
                                          decoration: InputDecoration(border: OutlineInputBorder(), contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 12), suffixText: 'DT'),
                                        )),
                                      ),
                                    ],
                                  ),
                                  ...
                                  _buildFormField('Notes', TextField(
                                    controller: _notesCtrl,
                                    maxLines: 3,
                                    decoration: InputDecoration(hintText: 'Saisir les notes du paiement', border: OutlineInputBorder(), contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 12)),
                                  )),
                                ],
                              ),
                            ),
