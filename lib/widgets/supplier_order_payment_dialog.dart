...
class SupplierOrderPaymentDialog extends StatefulWidget {
  ...
  const SupplierOrderPaymentDialog({Key? key, required this.supplierOrder}) : super(key: key);

  @override
  State<SupplierOrderPaymentDialog> createState() => _SupplierOrderPaymentDialogState();
}

class _SupplierOrderPaymentDialogState extends State<SupplierOrderPaymentDialog> {
  ...
  void _updateAmountField() {
    double amount = widget.supplierOrder.totalTTC + widget.supplierOrder.timbreFiscal - 0.0;
    if (_applyWithholdingTax) {
      double taxAmount = ((widget.supplierOrder.totalTTC + widget.supplierOrder.timbreFiscal) * _withholdingTaxRate) / 100;
      amount -= taxAmount;
    }
  ...
  void _save() {
    ...
    final payment = Payment(
      ...
      contactId: widget.supplierOrder.supplierId,
      contactType: 'customer',
      contactName: widget.supplierOrder.supplierName,
      amount: parsedAmount,
    ...
    final treasuryTx = TreasuryTransaction(
      ...
      description: 'Paiement de la facture ${widget.supplierOrder.number}',
    ...
    // Update SupplierOrder status and amount paid
    double taxAmount = _applyWithholdingTax ? ((widget.supplierOrder.totalTTC + widget.supplierOrder.timbreFiscal) * _withholdingTaxRate) / 100 : 0;
    ...
    if (_applyWithholdingTax && taxAmount > 0) {
      ...
      final rsPayment = Payment(
        ...
        accountId: _selectedAccountId,
        reference: widget.supplierOrder.number,
      ...
      final rsTreasuryTx = TreasuryTransaction(
        ...
        description: 'Retenue à la source ($_withholdingTaxRate%) pour la facture ${widget.supplierOrder.number}',
    ...
    String newStatus = widget.supplierOrder.status;
    double totalDue = widget.supplierOrder.totalTTC + widget.supplierOrder.timbreFiscal;
    
    if (newAmountPaid >= totalDue - 0.01) { // 0.01 tolerance for floating point issues
      newStatus = 'paid';
    }

    final updatedSupplierOrder = widget.supplierOrder.copyWith(
      status: newStatus,
    );
    context.read<SupplierOrdersBloc>().add(UpdateSupplierOrder(updatedSupplierOrder));
  ...
  Widget build(BuildContext context) {
    double remainingAmount = widget.supplierOrder.totalTTC + widget.supplierOrder.timbreFiscal - 0.0;
    double taxAmount = _applyWithholdingTax ? ((widget.supplierOrder.totalTTC + widget.supplierOrder.timbreFiscal) * _withholdingTaxRate) / 100 : 0;
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          Row(
                            children: [
                              Expanded(child: Text('Réf: ${widget.supplierOrder.number}', style: const TextStyle(fontWeight: FontWeight.w500))),
                              Expanded(child: Text('Date: ${formatDateTime(widget.supplierOrder.date)}')),
                            ],
                          ),
                          const SizedBox(height: 12),
                          Row(
                            children: [
                              Expanded(child: Text('Contact: ${widget.supplierOrder.supplierName ?? '—'}')),
                              Expanded(child: Text('Statut: ${widget.supplierOrder.status}')),
                            ],
                          ),
                          const SizedBox(height: 12),
                          Row(
                            children: [
                              Expanded(child: Text('Montant total: ${formatCurrencyDT(widget.supplierOrder.totalTTC + widget.supplierOrder.timbreFiscal)}', style: const TextStyle(fontWeight: FontWeight.w600))),
                              Expanded(
                                child: Row(
                                  children: [
                          ...
                          children: [
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  Text('Statut: ${widget.supplierOrder.status}', style: const TextStyle(fontSize: 13, color: AppColors.textPrimary)),
