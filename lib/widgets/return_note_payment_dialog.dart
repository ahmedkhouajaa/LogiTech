...
class ReturnNotePaymentDialog extends StatefulWidget {
  ...
  const ReturnNotePaymentDialog({Key? key, required this.returnNote}) : super(key: key);

  @override
  State<ReturnNotePaymentDialog> createState() => _ReturnNotePaymentDialogState();
}

class _ReturnNotePaymentDialogState extends State<ReturnNotePaymentDialog> {
  ...
  void _updateAmountField() {
    double amount = widget.returnNote.totalTTC.abs() - 0.0;
    if (_applyWithholdingTax) {
      double taxAmount = ((widget.returnNote.totalTTC.abs()) * _withholdingTaxRate) / 100;
      amount -= taxAmount;
    }
  ...
  void _save() {
    ...
    final payment = Payment(
      ...
      contactId: widget.returnNote.customerId,
      contactType: 'customer',
      contactName: widget.returnNote.customerName,
      amount: parsedAmount,
    ...
    final treasuryTx = TreasuryTransaction(
      ...
      description: 'Paiement de la facture ${widget.returnNote.returnNumber}',
    ...
    // Update ReturnNote status and amount paid
    double taxAmount = _applyWithholdingTax ? ((widget.returnNote.totalTTC.abs()) * _withholdingTaxRate) / 100 : 0;
    ...
    if (_applyWithholdingTax && taxAmount > 0) {
      ...
      final rsPayment = Payment(
        ...
        accountId: _selectedAccountId,
        reference: widget.returnNote.returnNumber,
      ...
      final rsTreasuryTx = TreasuryTransaction(
        ...
        description: 'Retenue à la source ($_withholdingTaxRate%) pour la facture ${widget.returnNote.returnNumber}',
    ...
    String newStatus = widget.returnNote.status;
    double totalDue = widget.returnNote.totalTTC.abs();
    ...
    final updatedReturnNote = widget.returnNote.copyWith(
      status: newStatus,
    );
    context.read<ReturnNotesBloc>().add(UpdateReturnNote(updatedReturnNote));
  ...
  Widget build(BuildContext context) {
    double remainingAmount = widget.returnNote.totalTTC.abs() - 0.0;
    double taxAmount = _applyWithholdingTax ? ((widget.returnNote.totalTTC.abs()) * _withholdingTaxRate) / 100 : 0;
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          Row(
                            children: [
                              Expanded(child: Text('Réf: ${widget.returnNote.returnNumber}', style: const TextStyle(fontWeight: FontWeight.w500))),
                              Expanded(child: Text('Date: ${formatDateTime(widget.returnNote.dateEmission)}')),
                            ],
                          ),
                          const SizedBox(height: 12),
                          Row(
                            children: [
                              Expanded(child: Text('Contact: ${widget.returnNote.customerName ?? '—'}')),
                              Expanded(child: Text('Statut: ${widget.returnNote.status}')),
                            ],
                          ),
                          const SizedBox(height: 12),
                          Row(
                            children: [
                              Expanded(child: Text('Montant total: ${formatCurrencyDT(widget.returnNote.totalTTC.abs())}', style: const TextStyle(fontWeight: FontWeight.w600))),
                              Expanded(
                                child: Row(
                                  children: [
                          ...
                          children: [
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  Text('Statut: ${widget.returnNote.status}', style: const TextStyle(fontSize: 13, color: AppColors.textPrimary)),
