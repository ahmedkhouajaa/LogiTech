...
class PurchaseInvoice {
  ...
  PurchaseInvoice({
    required this.id, required this.number, required this.supplierId,
    this.supplierName, this.orderId, required this.date, required this.dueDate,
    this.status = InvoiceStatus.draft, this.totalHT = 0, this.totalTva = 0,
    this.totalTTC = 0, this.amountPaid = 0, this.notes,
    this.items = const [], this.firebaseUid, this.isDeleted = false,
    DateTime? createdAt, DateTime? updatedAt,
  })  : createdAt = createdAt ?? DateTime.now(),
        updatedAt = updatedAt ?? DateTime.now();

  double get amountRemaining => totalTTC - amountPaid;

  Map<String, dynamic> toMap() => {
        'id': id, 'number': number, 'supplier_id': supplierId,
        'order_id': orderId, 'date': date.toIso8601String(),
        'due_date': dueDate.toIso8601String(), 'status': status.name,
        'total_ht': totalHT, 'total_tva': totalTva, 'total_ttc': totalTTC,
        'amount_paid': amountPaid, 'notes': notes,
        'firebase_uid': firebaseUid, 'is_deleted': isDeleted ? 1 : 0,
        'created_at': createdAt.toIso8601String(),
        'updated_at': updatedAt.toIso8601String(),
      };
  ...
  PurchaseInvoice copyWith({
    ...
    String? orderId, DateTime? date, DateTime? dueDate, InvoiceStatus? status,
    double? totalHT, double? totalTva, double? totalTTC, double? amountPaid,
    String? notes, List<PurchaseInvoiceItem>? items, String? firebaseUid,
    bool? isDeleted, DateTime? createdAt, DateTime? updatedAt,
        ...
        dueDate: dueDate ?? this.dueDate, status: status ?? this.status,
        totalHT: totalHT ?? this.totalHT, totalTva: totalTva ?? this.totalTva,
        totalTTC: totalTTC ?? this.totalTTC, amountPaid: amountPaid ?? this.amountPaid,
...
class PurchaseInvoiceItem {
  ...
  PurchaseInvoiceItem({
    required this.id, required this.invoiceId, required this.productId,
    this.productName, this.quantity = 1, this.unitPrice = 0,
    this.tvaRate = 19, this.totalHT = 0,
  });

  double get computedTotalHT => quantity * unitPrice;

  Map<String, dynamic> toMap() => {
        'id': id, 'invoice_id': invoiceId, 'product_id': productId,
        'quantity': quantity, 'unit_price': unitPrice, 'tva_rate': tvaRate,
