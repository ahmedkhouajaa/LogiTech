...
class _CustomerOrdersScreenState extends State<CustomerOrdersScreen> {
  ...
  void _applyFilters() {
    ...
    context.read<CustomerOrdersBloc>().add(FilterCustomerOrders(
      ...
      dateFrom: _dateFrom,
      dateTo: _dateTo,
      status: _statusFilter != null ? _statusFilter!.name : null,
    ));
    setState(() {
      _currentPage = 0;
    });
                          ...
                          child: Row(
                            children: [
                              SizedBox(
                                ...
                                height: 28,
                                child: Checkbox(
                                  value: paginatedOrders.isNotEmpty && paginatedOrders.every((o) => _selectedOrderIds.contains(o.id)),
                                  onChanged: (val) {
                                    setState(() {
                                      if (val == true) {
                                        _selectedOrderIds.addAll(paginatedOrders.map((o) => o.id));
                                      } else {
  ...
  void _handleBulkAction(String action) {
    final state = context.read<CustomerOrdersBloc>().state;
    if (state is! CustomerOrdersLoaded) return;

    final selectedOrders = state.orders.where((o) => _selectedOrderIds.contains(o.id)).toList();
  ...
  Future<void> _bulkDownloadPdf(List<CustomerOrder> selectedOrders) async {
    for (final o in selectedOrders) {
      final docWrapper = DocumentWrapper.fromCustomerOrder(o);
  ...
  Future<void> _bulkExportExcel(List<CustomerOrder> selectedOrders) async {
    try {
      final excel = Excel.createExcel();
      final sheet = excel['Commandes'];
      excel.setDefaultSheet('Commandes');
      ...
      for (var i = 0; i < selectedOrders.length; i++) {
        ...
        sheet.cell(CellIndex.indexByColumnRow(columnIndex: 2, rowIndex: rowIndex)).value = TextCellValue(o.customerCompany ?? o.customerName ?? '—');
      ...
      if (fileBytes != null && mounted) {
        final fileName = 'Commandes_Selectionnees_${DateTime.now().millisecondsSinceEpoch}.xlsx';
  ...
  void _bulkDeleteSelected(List<CustomerOrder> selectedOrders) {
    showDialog(
      context: context,
      builder: (dialogCtx) => AlertDialog(
        title: Row(
        ...
        content: Text('Voulez-vous vraiment supprimer ${selectedOrders.length} commande(s) sélectionnée(s) ?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(dialogCtx),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.pop(dialogCtx);
              for (final o in selectedOrders) {
                context.read<CustomerOrdersBloc>().add(DeleteCustomerOrder(o.id));
              }
              setState(() => _selectedOrderIds.clear());
              ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                content: Text('${selectedOrders.length} commande(s) supprimée(s)'),
                backgroundColor: AppColors.success,
              ));
            },
