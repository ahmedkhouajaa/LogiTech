...
class MobileSupplierReturnFormScreen extends StatefulWidget {
  final SupplierReturn? existing;
  final bool isReadOnly;
  const MobileSupplierReturnFormScreen({super.key, this.existing, this.isReadOnly = false});

  @override
  State<MobileSupplierReturnFormScreen> createState() => _MobileSupplierReturnFormScreenState();
}

class _MobileSupplierReturnFormScreenState extends State<MobileSupplierReturnFormScreen> {
  ...
  void _showArticleForm([int? index]) async {
    ...
    if (result != null) {
      setState(() {
        final newItem = SupplierReturnItem(
          id: index != null ? _items[index].id : _uuid.v4(),
          supplierReturnId: widget.existing?.id ?? '',
          productId: result.productId,
          designation: result.description.isNotEmpty ? result.description : (result.productName.isNotEmpty ? result.productName : 'Article'),
