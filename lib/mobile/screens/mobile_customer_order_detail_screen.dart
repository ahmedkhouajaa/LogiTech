...
class _MobileCustomerOrderDetailScreenState extends State<MobileCustomerOrderDetailScreen> {
  ...
  Future<void> _loadFullCustomerOrder() async {
  ...
  Widget build(BuildContext context) {
    ...
    final infoSections = [
      PremiumInfoSection(
        ...
        fields: [
          PremiumInfoField(
            label: 'Client',
            value: currentCustomerOrder.customerName ?? 'Inconnu',
            icon: Icons.person_outline,
            isHighlight: true,
          ),
          PremiumInfoField(
            label: 'Date de commande',
            value: formatDateTimeLong(currentCustomerOrder.date),
            icon: Icons.calendar_today_outlined,
          ),
          if (currentCustomerOrder.deliveryDate != null)
            PremiumInfoField(
              label: 'Date de livraison prévue',
              value: formatDateTimeLong(currentCustomerOrder.deliveryDate!),
              icon: Icons.local_shipping_outlined,
            ),
          if (currentCustomerOrder.projectName != null && currentCustomerOrder.projectName!.isNotEmpty)
            PremiumInfoField(
              label: 'Projet',
              value: currentCustomerOrder.projectName!,
    ...
    final articles = currentCustomerOrder.items.map((item) {
    ...
    final stampTax = currentCustomerOrder.timbreFiscal;
    final totals = <PremiumTotalRow>[
      PremiumTotalRow(
        label: 'Total HT',
        amount: currentCustomerOrder.subTotalHT,
      ),
      PremiumTotalRow(
        label: 'Total TVA',
        amount: currentCustomerOrder.totalTVA,
      ),
      if (stampTax > 0.01)
        PremiumTotalRow(
      ...
      PremiumTotalRow(
        label: 'Total TTC',
        amount: currentCustomerOrder.totalTTC,
        isGrandTotal: true,
      ),
    ];
    ...
    return BlocListener<CustomerOrdersBloc, CustomerOrdersState>(
      listener: (context, state) {
        if (state is CustomerOrdersLoaded) {
          try {
            final updated = state.orders.firstWhere((q) => q.id == currentCustomerOrder.id);
            setState(() {
              currentCustomerOrder = updated;
            });
          } catch (_) {
            if (!_isPopping) {
              _isPopping = true;
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: Text('Commande ${currentCustomerOrder.number}', style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
        ...
        body: PremiumDetailShell(
          documentType: 'Commande Client',
          referenceNumber: currentCustomerOrder.number,
          statusLabel: statusLabel,
          statusColor: statusColor,
          infoSections: infoSections,
          articles: articles,
          totals: totals,
          notes: currentCustomerOrder.notes,
          termsAndConditions: currentCustomerOrder.conditionsGenerales,
        ),
      ),
    );
  }
